...
struct PersonMessages: Codable {
    ...
    init(personsModel: [Player]) {
        personsModel.forEach { (person) in
            guard let position = person.lastPosition else { return }
            let person = PersonMessage(points: person.points,
                          colorIndex: PersonMessages.color.index(of: person.color)!,
